...
for coin in coins:
    ...
    pd.DataFrame(predictions, index=model.df["2020-05-11":].iloc[:-model.N_PERIOD, :].index).to_csv(
        f"models/predictions/lstm_price_change_pred_{coin}.csv", header=None)
...
def dateparse(dates): return pd.datetime.strptime(
    dates[:dates.find('+')], '%Y-%m-%d %H:%M:%S')


df_btc = pd.read_csv('btc_metrics_raw.csv', index_col='Date',
                     parse_dates=True, date_parser=dateparse)
df_eth = pd.read_csv('eth_metrics_raw.csv', index_col='Date',
                     parse_dates=True, date_parser=dateparse)
df_ltc = pd.read_csv('ltc_metrics_raw.csv', index_col='Date',
                     parse_dates=True, date_parser=dateparse)
...
data = {'Volatility': [btc_HV, eth_HV, ltc_HV], "Annualised Returns": [
    btc_returns, eth_returns, ltc_returns]}
stats_df = pd.DataFrame(data).round(2)
stats_df.index = ['btc', 'eth', 'ltc']

for coin in coins:
    Lstm_obj = LSTMModel(f'{coin}_metrics_raw.csv',
                         f'models/saved_models/{coin}/lstm_price_predictor.hp5')
    Arima_Obj = ArimaModel(f'{coin}_metrics_raw.csv',
                           10, f'models/saved_models/{coin}/arimamodel.pkl')

    def dateparse(dates): return pd.datetime.strptime(
        dates[:dates.find('+')], '%Y-%m-%d %H:%M:%S')
    df = pd.read_csv(f'{coin}_metrics_raw.csv', index_col='Date',
                     parse_dates=True, date_parser=dateparse)

    df_ma_20 = moving_avg(df['close'], 20)
    df_ma_50 = moving_avg(df['close'], 50)

    df_ma = df_ma_20[-60:].reset_index().merge(df_ma_50[-60:].reset_index(),
                                               on='Date').rename(columns={'close_x': 'MA_20', 'close_y': 'MA_50'})

    df_close_last_60 = df['close'][-60:].reset_index()

    Lstm_pred = Lstm_obj.forecast().reset_index().rename(
        columns={0: 'LSTM', 'index': 'Date'})

    Arima_pred = Arima_Obj.arima_pred_future(
